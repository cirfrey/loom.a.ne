...
auto lm::get_version_adjective(u8 major, u8 minor) -> char const* { return adjectives[(major + minor) % 32]; }
auto lm::get_version_subject(u8 major, u8 minor)   -> char const* { return subjects[(minor + major * 13) % 32]; }
...
auto lm::write_banner(
    banner_writer_t writer,
    banner_sleeper_t sleeper,
    text prefix,
    banner_data data
) -> void
{
