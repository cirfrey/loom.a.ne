...
auto lm::fabric::resolve_to_name(u8 id, u64 max_tries, u64 timeout_ms) -> u32
{
    ...
    auto q   = fabric::queue<fabric::event>(1);
    auto tok = fabric::bus::subscribe(
        q,
        framework::topic,
        {framework::response_manager_resolve::type},
        [](void* params, std::span<fabric::event> events){
            auto id = params | unsmuggle<u8>;
            auto data = events[0].get_payload<framework::response_manager_resolve>();
            return data.requester_id == id
                ? fabric::bus::pass
                : fabric::bus::filter;
        },
        id | smuggle<void*>
