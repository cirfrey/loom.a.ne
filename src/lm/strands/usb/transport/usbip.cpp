...
auto lm::strands::usb::transport::usbip::on_wake() -> status
{
...
lm::strands::usb::transport::usbip::~usbip() { transition_state(initializing); }

/// State stuff.

auto lm::strands::usb::transport::usbip::transition_state(state_t to) -> bool
{
...
auto lm::strands::usb::transport::usbip::do_handshaking_state() -> desired_strand_action
{
...
auto lm::strands::usb::transport::usbip::handshaking_read_header() -> void
{
    auto& data = state_data.handshaking;
...
auto lm::strands::usb::transport::usbip::handshaking_read_busid() -> void
{
    auto& data = state_data.handshaking;
...
auto lm::strands::usb::transport::usbip::handshaking_process_req_devlist() -> void
{
    auto& data = state_data.handshaking;
    ...
    std::snprintf(dev_info.busid, sizeof(dev_info.busid), "%s", cfg->busid);
    ...
    data.status = cfg->close_conn_after_devlist == feature::on
...
// different header.
auto lm::strands::usb::transport::usbip::handshaking_process_req_import() -> void
{
    auto& data = state_data.handshaking;
    ...
    std::snprintf(pkt.device.path,  sizeof(pkt.device.path),  "%s", cfg->path);
    std::snprintf(pkt.device.busid, sizeof(pkt.device.busid), "%s", cfg->busid);
...
auto lm::strands::usb::transport::usbip::do_exported_state() -> desired_strand_action
{
...
auto lm::strands::usb::transport::usbip::do_transmitting_state() -> desired_strand_action
{
...
auto lm::strands::usb::transport::usbip::transmitting_read_urb() -> bool
{
...
auto lm::strands::usb::transport::usbip::transmitting_handle_unlink(const lm::usbip::usbip_cmd_unlink& unlink) -> bool
{
    auto& data = state_data.transmitting;
...
auto lm::strands::usb::transport::usbip::transmitting_flush_inbound_events() -> bool
{
    auto& data = state_data.transmitting;
...
auto lm::strands::usb::transport::usbip::transmitting_handle_setup(
    const lm::usbip::usbip_cmd_submit& cmd, u32 seqnum, [[maybe_unused]] u8 ep_num) -> bool
{
    ...
    return send_ret_submit(seqnum, 0, lm::usbip::USBIP_DIR_IN, cfg->stall_status_code, nullptr, 0);
}

auto lm::strands::usb::transport::usbip::transmitting_handle_in(
...
auto lm::strands::usb::transport::usbip::transmitting_handle_out(
    const lm::usbip::usbip_cmd_submit& cmd, u32 seqnum, u8 ep_addr) -> bool
{
    ...
    //
    //   CDC bulk OUT endpoint → publish toic::cdc_data event
...
auto lm::strands::usb::transport::usbip::setup_handle_get_descriptor(const u8 setup[8], u32 seqnum) -> bool
{
...
auto lm::strands::usb::transport::usbip::send_ret_submit(
...
auto lm::strands::usb::transport::usbip::serialise_event_to_usb(
